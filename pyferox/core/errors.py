...
class PyFerOxError(FrameworkError):
    """Backward-compatible framework base error."""
...
class InfrastructureError(PyFerOxError):
    pass


def normalize_execution_exception(exc: Exception) -> Exception:
    if isinstance(exc, FrameworkError):
        return exc
    return InfrastructureError("Unhandled infrastructure failure")
...
def map_exception_to_transport(exc: Exception) -> tuple[int, dict[str, Any]]:
    ...
    if isinstance(exc, PermissionDeniedError):
        return 403, _error_payload("permission_denied", str(exc) or "Forbidden")
    if isinstance(exc, UnauthorizedError):
        return 401, _error_payload("unauthorized", str(exc) or "Unauthorized")
    if isinstance(exc, AuthError):
        return 401, _error_payload("auth_error", str(exc) or "Unauthorized")
    if isinstance(exc, InfrastructureError):
        return 500, _error_payload("infrastructure_error", "Infrastructure error")
